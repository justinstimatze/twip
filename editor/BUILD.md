...
`desktop.yml` builds a `.deb` on pushes to main that touch `editor/`, `src/` or the cargo
manifests, and leaves it as a run artifact. To install the newest one:

```
scripts/install-desktop.sh
```

Build and install from the working tree instead with `--local`.
